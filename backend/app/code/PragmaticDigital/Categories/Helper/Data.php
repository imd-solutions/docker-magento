...
namespace PragmaticDigital\Categories\Helper;
...
class Data extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    {
        $subCatImage = $this->getCategory($catId)->getPragmaticDigitalCategoryImage();
        if ($subCatImage) {
            $arr     = explode('/media/', $subCatImage);
            $imgPath = $this->getMediaUrl() . $arr[1];
        } else {
            $imgPath = $this->getMediaUrl() . 'PragmaticDigital_categories/no-photo.jpg';
        }
        return $imgPath;
    }
}
