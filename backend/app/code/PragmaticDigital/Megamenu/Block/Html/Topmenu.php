...
namespace PragmaticDigital\Megamenu\Block\Html;
...
class Topmenu extends \Magento\Theme\Block\Html\Topmenu
{
    ...
    protected function _addSubMenu($child, $childLevel, $childrenWrapClass, $limit)
    {
        ...
        $html .= '<div class="dropdowm-megamenu ' . $catColumnClass . ' ' . $type . '" style="' . $dropdownWidthStyle . '"><div class="container">'; // div dropdown
        ...
        if ($enableLeft || $enableRight || $child->hasChildren()) {
            $html .= '<div class="megamenu-middle middle-level-' . $childLevel . '"><div class="row">'; // div middle

            $leftClass = '';
            if ($enableLeft && $enableMegamenu && $childLevel == 0) {
                $leftClass = 'offset-md-2';
                $html .= '<div class="col-12 col-md-6"><div class="megamenu-content-left">' . $contentLeft . '</div></div>';
            }

            if ($child->hasChildren()) {
                $html .= '<div class="col-12 col-md-4 ' . $leftClass . '">';
                $html .= '<div class="megamenu-content-cate">';
            ...
            if ($enableRight && $enableMegamenu && $childLevel == 0) {
                $html .= '<div class="col-12 col-md-6 offset-md-2"><div class="megamenu-content-right">' . $contentRight . '</div></div>';
            }

            $html .= '</div></div>'; // end div middle
        }
    ...
    protected function _getHtml(
        ...
        $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
        $helper        = $objectManager->get('\PragmaticDigital\Megamenu\Helper\Data');
        ...
        foreach ($children as $child) {
            ...
            $html .= '<a href="' . $objectManager->create('Magento\Framework\Escaper')->escapeHtml($catLinkUrl) . '" ' . $outermostClassCode . '>' . $this->getLabel($child) . ' ' . $this->getIcon($child) . '<span class="cat-name">' . $this->escapeHtml(
                $child->getName()
